    ...
    <section id="timeline" className="py-20 lg:py-32 bg-sky-600 text-white rounded-[2.5rem] sm:rounded-[4rem] mx-2 sm:mx-4 my-20">
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12 relative">
          ...
          {[
            ...
            <motion.div 
              key={i}
              whileHover={{ y: -15 }}
              className="bg-white/10 backdrop-blur-xl p-8 lg:p-10 rounded-[2rem] sm:rounded-[3rem] border border-white/20 relative z-10 text-center"
            >
        ...
        <div className="mt-16 lg:mt-24 max-w-3xl mx-auto">
          ...
          >
            <p className="text-lg sm:text-xl font-bold mb-4 line-through opacity-50">HỌC PHÍ GỐC: 10,500,000Đ</p>
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black mb-6 text-sky-600 tracking-tighter">MIỄN PHÍ 0Đ</h2>
            <p className="text-base sm:text-xl font-bold mb-10 text-slate-600 leading-relaxed uppercase">Trải nghiệm khóa học 2 ngày đầu tiên để thấy sự thay đổi!</p>
            <p className="text-red-500 font-black mb-10 text-base sm:text-lg flex items-center justify-center gap-2 animate-bounce">
              <Clock className="w-6 h-6"/> CHỈ CÒN 20 SUẤT CUỐI CÙNG TRONG THÁNG 3
            </p>
            <button className="w-full sm:w-auto px-12 py-5 sm:py-6 bg-sky-500 text-white rounded-full font-black text-xl sm:text-2xl hover:bg-sky-600 transition-all hover:scale-105 shadow-2xl shadow-sky-200">
